...
// --- Homepage Section 3 ("Worth the Roam") controlled population --------
// Same idempotent seed discipline as the Hidden Gems block above, reusing
// the exact same `collections`/`collection_items` tables with a new
// `kind` value — precisely the reuse the schema's own comment above
// anticipated ("a future Roam Picks collection can reuse this same table
// without a schema change"). No new table, no schema migration.
//
// Positions 1-2 are the section's two "featured" cards (larger in the
// homepage layout); positions 3-6 are the four supporting cards. Every
// `note` below is a single editorial sentence grounded only in that
// venue's own existing `description` field (verified against live
// production data before writing) — no invented claim, award, ownership
// detail, or atmosphere not already stated there.
const WORTH_THE_ROAM_COLLECTION_SLUG = 'worth-the-roam';
const WORTH_THE_ROAM_MEMBERS = [
  { position: 1, venue_id: 1029, note: "Tinhorn Creek Vineyards' fine-dining restaurant, known for some of the best views in the South Okanagan and a black pepper carbonara worth the trip alone." },
  { position: 2, venue_id: 368, note: 'A working botanical garden in Kaleden with a cafe attached — wander past goats and bunnies on your way to a club sandwich and a homemade oat-milk London fog.' },
  { position: 3, venue_id: 740, note: "A family-friendly West Kelowna grill built around chicken rolls and a lively happy hour, just as welcoming if you're dining solo." },
  { position: 4, venue_id: 824, note: 'An appointment-only Oliver winery focused entirely on chardonnay and merlot, with a patio regulars call the best in the Okanagan.' },
  { position: 5, venue_id: 289, note: 'Elegant fine-dining Italian in Vernon, with a genuine gluten-free French onion soup and live Spanish guitar on select nights.' },
  { position: 6, venue_id: 863, note: 'A well-known Naramata Bench winery with sweeping views, a wide-ranging wine list, and tasting-room staff regulars describe as refreshingly unpushy.' },
];

let worthTheRoamCollection = db
  .prepare('SELECT id FROM collections WHERE slug = ?')
  .get(WORTH_THE_ROAM_COLLECTION_SLUG);

if (!worthTheRoamCollection) {
  const info = db
    .prepare('INSERT INTO collections (slug, kind, title, region) VALUES (?, ?, ?, NULL)')
    .run(WORTH_THE_ROAM_COLLECTION_SLUG, 'worth_the_roam', 'Worth the Roam');
  worthTheRoamCollection = { id: info.lastInsertRowid };
}

for (const member of WORTH_THE_ROAM_MEMBERS) {
  const venue = db
    .prepare('SELECT id FROM venues WHERE id = ? AND redirect_to IS NULL')
    .get(member.venue_id);
  if (!venue) continue; // venue doesn't exist (or is redirected) in this database — skip, don't fabricate membership
  ...
  const existingItem = db
    .prepare("SELECT 1 FROM collection_items WHERE collection_id = ? AND content_type = 'venue' AND content_id = ?")
    .get(worthTheRoamCollection.id, member.venue_id);
  if (existingItem) continue; // already seeded — idempotent no-op

  db.prepare(`
    INSERT INTO collection_items (collection_id, content_type, content_id, note, position)
    VALUES (?, 'venue', ?, ?, ?)
  `).run(worthTheRoamCollection.id, member.venue_id, member.note, member.position);
}
